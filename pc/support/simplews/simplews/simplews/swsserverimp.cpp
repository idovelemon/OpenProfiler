...
SWSServerImp::SWSServerImp(int16_t port)
...
SWSServerImp::~SWSServerImp() {
	Close();
}

SWSCommunication* SWSServerImp::WaitConnection(int32_t time) {
	bool result = false;
	...
	SWSCommunication* comm = new SWSCommunication(comm_imp);

	return comm;
}

void SWSServerImp::Close() {
